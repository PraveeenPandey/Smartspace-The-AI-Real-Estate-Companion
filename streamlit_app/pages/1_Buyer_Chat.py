...
st.title("Buyer Chat")
st.caption("Capture buyer intent, persist the session, and produce shortlist-ready recommendations.")
...
with left:
    ...
    with st.form("buyer_recommendation_form"):
        city = st.text_input("City", value=preferences.get("city", "Bangalore"))
        col1, col2 = st.columns(2)
        with col1:
            budget_min = st.number_input("Budget Min", min_value=1000000, value=9000000, step=500000)
        with col2:
            budget_max = st.number_input("Budget Max", min_value=1000000, value=15000000, step=500000)
        col3, col4 = st.columns(2)
        with col3:
            bhk = st.number_input("BHK", min_value=1, max_value=10, value=preferences.get("bhk", 3))
        with col4:
        ...
        with col5:
            purpose = st.selectbox("Purpose", ["self_use", "investment"])
        with col6:
            commute_anchor = st.text_input("Commute Anchor", value="nearest metro")
        search_submitted = st.form_submit_button("Generate Shortlist")
...
with right:
    ...
    if st.session_state.buyer_session_id:
        try:
            ...
            st.caption(f"Session #{session_detail['session_id']} | stage: {session_detail['workflow_stage']}")
            for message in session_detail["messages"]:
                with st.chat_message(message["sender_type"]):
                    ...
                    if message["extracted_entities"]:
                        st.caption(f"Entities: {message['extracted_entities']}")
        except Exception as exc:
            st.error(f"Could not load session trace: {exc}")
    else:
        st.info("Analyze a buyer request to create a persisted session.")
